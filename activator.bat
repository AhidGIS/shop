...
@REM JAVA8_HOME - location of a JDK home dir (optional if java on path)
...
if "%_JAVACMD%"=="" (
  if not "%JAVA8_HOME%"=="" (
    if exist "%JAVA8_HOME%\bin\java.exe" set "_JAVACMD=%JAVA8_HOME%\bin\java.exe"
...
if "%_JAVACCMD%"=="" (
  if not "%JAVA8_HOME%"=="" (
    if exist "%JAVA8_HOME%\bin\javac.exe" set "_JAVACCMD=%JAVA8_HOME%\bin\javac.exe"
  )
)
...
if "%JAVAOK%"=="false" (
  ...
  echo available via JAVA8_HOME or PATH.
  echo.
